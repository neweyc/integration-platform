...
Integrations can be pinned to uploaded package versions, and execution records retain the package id/name/version used for each run. The integration history page shows the active version and lets an operator (with `ManageIntegrations`) repoint it to another version of the same package via `PUT /api/integrations/{id}/package`; this changes only the pin and takes effect on the next run. A general integration update never changes the pin. A dedicated Packages page lists packages grouped by name, marks each version **in use** (with the integrations pinning it) or **stale**, and allows deleting stale versions only — deletion of an in-use version is blocked (`409`) because the pin uses `OnDelete(SetNull)` and would otherwise silently un-pin the integration. Current operational limitations: packages are not environment-scoped, package deletion does not remove local agent cache entries, and package assemblies are isolated per version but not yet unloaded (old versions accumulate on a long-running agent).
