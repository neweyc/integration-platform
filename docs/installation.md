...
> **Note:** `serto init` scaffolds a project that references the published `Serto.Sdk`, `Serto.Connectors`, and `Serto.Testing` NuGet packages. The current package version is `1.4.1`:
>
> ```xml
> <ItemGroup>
>   <PackageReference Include="Serto.Sdk" Version="1.4.1" />
>   <PackageReference Include="Serto.Connectors" Version="1.4.1" />
> </ItemGroup>
> ```
...
| `Maintenance:Enabled` | `Maintenance__Enabled` | Soft-launch circuit breaker (default `false`). When `true`, the control plane rejects every write (POST/PUT/PATCH/DELETE) with `503` so nothing can be written to the database — no tenant registration, no first-run setup, no data. Reads (the public site, docs, read-only UI, health) still work, and the landing page hides its "open app / sign in" entry points. Toggle with no rebuild; restart to apply. |
...
- **Scaffolded integration won't restore `Serto.Sdk`** — confirm NuGet.org is reachable and that the generated project references a published package version such as `1.4.1`.
