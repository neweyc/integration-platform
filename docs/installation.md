...
> **Note:** `serto init` scaffolds a project that references the published `Serto.Sdk`, `Serto.Connectors`, and `Serto.Testing` NuGet packages. The current package version is `1.2.0`:
>
> ```xml
> <ItemGroup>
>   <PackageReference Include="Serto.Sdk" Version="1.2.0" />
>   <PackageReference Include="Serto.Connectors" Version="1.2.0" />
> </ItemGroup>
> ```
...
- **Scaffolded integration won't restore `Serto.Sdk`** — confirm NuGet.org is reachable and that the generated project references a published package version such as `1.2.0`.
